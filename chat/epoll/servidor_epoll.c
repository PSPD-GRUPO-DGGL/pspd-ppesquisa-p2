/* Servidor de chat full-duplex com epoll em modo edge-triggered.
 *
 * Uso: servidor_epoll <porta> [--eco] [--metricas <porta>]
 *
 *   --eco       devolve a mensagem só ao remetente (usado no experimento C10K)
 *   --metricas  expõe /metrics em texto Prometheus na porta indicada
 */
...
static void ajustar_interesse(int fd, int quer_escrita) {
    if (conexoes[fd].quer_escrita == quer_escrita) return;
...
int main(int argc, char **argv) {
    ...
    for (int i = 2; i < argc; i++) {
        if (!strcmp(argv[i], "--eco")) modo = MODO_ECO;
    ...
    if (porta_metricas) {
        ...
        if (fd_metricas < 0) { perror("listen metricas"); return 1; }
    ...
    for (;;) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (fd == fd_escuta) { aceitar_todas(fd_escuta, modo); continue; }
            if (fd == fd_metricas) { servir_metricas(fd_metricas); continue; }

            if (e & (EPOLLHUP | EPOLLERR)) { fechar_conexao(fd); continue; }
            if (e & EPOLLOUT) { drenar_saida(fd); if (!conexoes[fd].em_uso) continue; }
            if (e & (EPOLLIN | EPOLLRDHUP)) ler_ate_eagain(fd, modo);
        }
    }
    return 0;
}
