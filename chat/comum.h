...
typedef enum { MODO_BROADCAST, MODO_ECO } Modo;

/* Buffer de saída por conexão: `write` pode aceitar menos bytes do que o
   pedido, e o resto precisa esperar o socket ficar gravável. */
typedef struct {
    char  *dados;
    size_t tam;
    size_t cap;
